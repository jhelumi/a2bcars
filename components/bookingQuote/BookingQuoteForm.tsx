...
const BookingForm: React.FC = () => {
  ...
  const handleFormSubmit = async (data: any) => {
    try {
      ...
      if (res.ok) {
        toast.success(`Thansk for the Information`, { position: 'top-center' });
        router.replace('/');
      }
    } catch (error) {
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="space-y-3">
          ...
          <select
            ...
            className="w-full p-3 rounded-xl border-2 border-slate-200 bg-white focus:border-indigo-600 outline-none"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full p-3 rounded-xl border border-slate-200 focus:ring-1 focus:ring-brand-blue outline-none"
          />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium text-slate-700">
      ...
      {journeyType === 'Airport' && (
        <div className="p-6 bg-slate-50 border border-indigo-100 rounded-2xl space-y-4 animate-in zoom-in-95 duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Heathrow / JFK"
                className="w-full p-2 rounded-lg border border-slate-200 outline-none"
              />
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-500 uppercase">
          ...
          {errors.airportName && (
            <p className="text-xs text-red-500">{errors.airportName.message}</p>
          )}
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="w-full bg-brand-blue hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-bold py-4 px-6 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2"
      >
        {isLoading ? (
