'use client';
import React from 'react';

const TripAdvisor = () => {
  return (
    <div>
      <div
        id="TA_cdsscrollingravenarrow72"
        className="TA_cdsscrollingravenarrow pt-6"
      >
        <ul id="Erk7W6rkw29z" className="TA_links DtauCSp">
          <li id="hJV10XKMNetS" className="oQetaw">
            <a
              target="_blank"
              href="https://www.tripadvisor.co.uk/Attraction_Review-g4208470-d17837562-Reviews-A2B_Cars-Long_Crendon_Aylesbury_Aylesbury_Vale_Buckinghamshire_England.html"
            >
              <img
                src="https://static.tacdn.com/img2/brand_refresh/Tripadvisor_lockup_vertical.svg"
                width="64"
                height="64"
                alt="TripAdvisor"
                className="widEXCIMG  text-blue-50"
                id="CDSWIDEXCLOGO"
              />
            </a>
          </li>
        </ul>
      </div>
      <script
        async
        src="https://www.jscache.com/wejs?wtype=cdsscrollingravenarrow&amp;uniq=72&amp;locationId=17837562&amp;lang=en_UK&amp;border=true&amp;backgroundColor=gray&amp;display_version=2"
        data-loadtrk
      ></script>
    </div>
  );
};

export default TripAdvisor;
